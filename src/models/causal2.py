...
def run_phase2(epochs, subject, phoneme_pair,
                ...
                alternate_predictor: np.ndarray,
                idxs_val, y_proba_val):
    """
    Run the second phase of the causal analysis.
    """
    epochs_i_B = epochs[subject][f"phoneme_pair == '{phoneme_pair}'"] \
        .pick(population_B) \
        .copy().crop(*population_B_window)

    X2 = epochs_i_B.get_data()
    X2 = X2.reshape(X2.shape[0], -1)  # (n_epochs, n_channels * n_times)
    X2 = X2[idxs_val]

    Y2a = alternate_predictor
    Y2b = metadata.iloc[idxs_val][primary_predictor].combine(0, max).values

    return stratified_nested_cv(X2, Y2b, Y2a, metadata, idxs_val)


def run_causal2_analysis(epochs, subject, phoneme_pair,
    ...
    # "cognitive mismatch": true if the MAP prediction of acoustic cue disagrees with the
    # ground-truth lexical evidence
    cognitive_mismatch_estimate = (metadata.iloc[idxs_val].lexical_evidence == y_proba_val.argmax(axis=1)).astype(int).values
    
    return run_phase2(
        epochs, subject, phoneme_pair,
        population_B, population_B_window,
        metadata,
        primary_predictor="mismatch",
        alternate_predictor=cognitive_mismatch_estimate,
        idxs_val=idxs_val,

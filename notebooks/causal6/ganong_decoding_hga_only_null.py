...
# %% [markdown]
# causal6: ganong HGA-only permutation-null refits.
#
# Runs K label-shuffled refits of the pooled-across-completions HGA-only
# ganong searchlight. Writes `null_scores.parquet` with columns:
...
null_scores = run_ganong_hga_only_permutations(
...
assert null_scores.height > 0, (
    f"[{subject}] ganong hga_only null run produced no rows"
)

...
# %% [markdown]
# Sync source: notebooks/causal6/ganong_decoding_single_electrode.py
#
# causal6: Ganong decoding with `resampled` control predictor — GPU-batched.
#
# Pools behavior trials across lexical completions (no within-word_end split)
# to expose the Ganong boundary shift on top of the acoustic step. Uses
# StratifiedKFold outer CV (5-fold) and a fixed L2 penalty
# `reg_lambda_ganong_full`.
#
# Two models per (electrode × phoneme_pair × window):
#   baseline: logreg on `resampled` only (fit ONCE per (phoneme_pair, fold))
#   full:     logreg on [resampled, HGA window]
#
# Output parquets use long format with a `model` column ("full" | "baseline").
# No `word_end` column — trials from both completions enter the same fit.
...
from src.models.causal6 import make_windows, run_ganong_with_control
...
reg_lambda = 1.0              # populated from reg_lambda_winners.json::reg_lambda_ganong_full
...
# %%
scores_df, predictions_df, coefficients_df = run_ganong_with_control(

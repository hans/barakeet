...
# Paths to behavioral decoder checkpoints (results.joblib), one per subject.
# Snakemake passes this as a list; default uses glob for interactive use.
behavioral_decoder_paths = list(
    Path("outputs/causal5/behavior_decoding_single_electrode").glob("*/results.joblib")
)

# Directories containing acoustic decoder outputs per subject.
# Each directory should contain: decoding_models.joblib, outcomes.parquet, all_outcomes.parquet.
# Snakemake passes derived directories from the decoding_models.joblib inputs.
acoustic_decoder_dirs = [
    str(p.parent)
    for p in Path("outputs/causal5/behavior_decoding_single_electrode_acoustic").glob(
        "*/decoding_models.joblib"
    )
]
...
outdir = "outputs/causal5/A_neurometrics"
...
# %%
# Load phonetic (acoustic searchlight) decoder checkpoints from separate files.
# Builds the structure expected by viz_paper.py evaluate_phonetic_transfer /
# evaluate_behav_decoder_on_phon_window:
#   checkpoints[subject] = {
#       "models":       dict[DecoderFitKey, list[estimator]],
#       "outcomes":     dict[DecoderFitKey, DataFrame],   # test-fold predictions
#       "all_outcomes": dict[DecoderFitKey + (measure,), DataFrame],
#   }
import joblib as _joblib

_acoustic_dir_map = {
    re.findall(r"(EC\d+)", str(d))[0]: Path(d)
    for d in acoustic_decoder_dirs
    if re.findall(r"(EC\d+)", str(d))
}

phonetic_decoder_checkpoints = {}
for subject in tqdm(epochs.keys()):
    acdir = _acoustic_dir_map[subject]
    models = _joblib.load(acdir / "decoding_models.joblib")

    outcomes_df = pd.read_parquet(acdir / "outcomes.parquet")
    outcomes = {
        key: group.reset_index(drop=True)
        for key, group in outcomes_df.groupby(
            ["subject", "electrode_idx", "phoneme_pair", "smin", "smax"]
        )
    }

    all_outcomes_df = pd.read_parquet(acdir / "all_outcomes.parquet")
    all_outcomes = {
        key: group.reset_index(drop=True)
        for key, group in all_outcomes_df.groupby(
            ["subject", "electrode_idx", "phoneme_pair", "smin", "smax", "measure"]
        )
    }

    phonetic_decoder_checkpoints[subject] = {
        "models": models,
        "outcomes": outcomes,
...
# %%
# Load behavioral decoder checkpoints from causal5 joblib files.
# causal5 saves with keys "decoding_results" / "decoders"; viz_paper.py
# evaluate_behav_decoder_on_phon_window expects "A_decoders", so normalize on load.
_behav_path_map = {
    re.findall(r"(EC\d+)", str(p))[0]: Path(p)
    for p in behavioral_decoder_paths
    if re.findall(r"(EC\d+)", str(p))
}

behavioral_decoder_checkpoints = {}
for subject in tqdm(epochs.keys()):
    raw = _joblib.load(_behav_path_map[subject])
    behavioral_decoder_checkpoints[subject] = {"A_decoders": raw["decoders"]}

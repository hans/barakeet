...
# %% [markdown]
# Summarize HGA-only Ganong-effect decoding results across all subjects — causal5 pipeline.
#
# Mirrors `ganong_decoding_summarize.py` with three deliberate deviations since
# the HGA-only fit has no baseline to compare against:
...
#   - no `diff` column in intermediate DataFrames
#
...
# %% tags=["parameters"]
# List of paths to `results.joblib` files from ganong_decoding_single_electrode_hga_only,
# one per subject.
result_paths = list(
    Path("outputs/causal5/ganong_decoding_hga_only").glob("*/results.joblib")
)
...
if all_decoder_predictions:
    ...
    ganong_predictions = ganong_predictions.drop(

# ---
# jupyter:
#   jupytext:
#     formats: ipynb,py:percent
#     text_representation:
#       extension: .py
#       format_name: percent
#       format_version: '1.3'
#       jupytext_version: 1.18.1
#   kernelspec:
#     display_name: Python 3
#     language: python
#     name: python3
# ---

# %% [markdown]
# Ganong-effect decoding from single electrodes — causal5 pipeline.
#
# Decodes `behavior_categorical_forced` (heard /d/ or /n/) **pooled across both
# lexical completions**, dropping the within-completion constraint used in
# `behavior_decoding_single_electrode`.  Pooling exposes the systematic Ganong
# boundary shift (completion shifts the perceptual boundary) alongside the
# trial-level perceptual variance captured by the within-completion decoder.
#
# Electrode set: restricted to sites that show significant behavioral decoding in
# `behavior_decoding_single_electrode_summarize` (full_roc_auc ≥ threshold at
# their peak window, averaged over folds).
#
# Stratification on `(resampled, lexical_evidence)` ensures each CV fold has
# balanced acoustic steps and completions, providing acoustic control without
# trial filtering.
#
# Outputs `results.joblib` with the same schema as `behavior_decoding_single_electrode`:
#   - `decoding_results`: dict[(subject, electrode_idx, phoneme_pair)] → DataFrame
#   - `decoders`:         dict[(subject, electrode_idx, phoneme_pair)] → decoder dict

# %%
import os

import joblib

os.environ["OMP_NUM_THREADS"] = "1"  # Limit OpenMP
os.environ["MKL_NUM_THREADS"] = "1"  # Limit MKL (Intel Math Kernel Library)
os.environ["OPENBLAS_NUM_THREADS"] = "1"  # Limit OpenBLAS
os.environ["NUMEXPR_MAX_THREADS"] = "1"  # Limit NumExpr if installed

# %%
import itertools
import re

import mne
import pandas as pd
from tqdm.auto import tqdm

# %%
from src.data import add_metadata_features
from src.models.decoding import run_decoding_model_comparison_population

# %% tags=["parameters"]
subject = "EC282"
epochs_path = f"outputs/epochs_preprocessed/{subject}_epo.fif"
electrodes_path = f"outputs/causal5/find_speech_responsive/{subject}_results.csv"
behav_summary_path = f"outputs/causal5/behavior_decoding_single_electrode_summarize/{subject}/A_final_summary.csv"
outdir = "."

min_sample = 1
window_size = 15
stride = 2
n_jobs = 5

# Minimum full-model ROC-AUC (averaged over folds at peak window) required for
# a site to be included. Mirrors behav_response_peak_threshold from config.yaml.
behav_peak_threshold = 0.6

# %%
subject = re.findall(r"(EC[\d]+)_epo", str(epochs_path))[0]

# %%
electrode_df = pd.read_csv(electrodes_path)
speech_responsive_idxs = sorted(
    electrode_df.loc[electrode_df.speech_responsive, "electrode_idx"].unique()
)

# %% [markdown]
# ## Filter to behaviorally significant sites
#
# Load `A_final_summary.csv` from `behavior_decoding_single_electrode_summarize`.
# A site passes if its peak-window full_roc_auc (averaged over word_end groups and
# folds) meets the threshold in any phoneme pair.

# %%
behav_summary = pd.read_csv(behav_summary_path)

sig_electrode_idxs = sorted(
    behav_summary.loc[
        behav_summary["full_roc_auc"] >= behav_peak_threshold, "electrode_idx"
    ]
    .astype(int)
    .unique()
)

# Restrict to those that are also speech-responsive (sanity check)
sig_electrode_idxs = [i for i in sig_electrode_idxs if i in speech_responsive_idxs]

# %%
epochs = mne.read_epochs(epochs_path, verbose=False)
assert epochs.metadata is not None
epochs.metadata = add_metadata_features(epochs.metadata)

# %%
max_sample = epochs.times.shape[0]
phoneme_pairs = sorted(epochs.metadata.phoneme_pair.dropna().unique())

# %% [markdown]
# ## Decode from behaviorally significant electrodes — pooled across completions
#
# Key difference from `behavior_decoding_single_electrode`:
#   - `groupby=None`  — pool trials from both completions (drops within-completion
#     constraint; exposes the Ganong boundary shift)
#   - `stratify=("resampled", "lexical_evidence")` — balance acoustic steps AND
#     completions across CV folds (acoustic control)
#   - Same target, baseline features, window parameters, and CV strategy.

# %%
A_decoding_results, A_decoders = {}, {}
for electrode_idx, phoneme_pair in tqdm(
    list(itertools.product(sig_electrode_idxs, phoneme_pairs))
):
    key = (subject, electrode_idx, phoneme_pair)
    A_decoding_results[key], A_decoders[key] = run_decoding_model_comparison_population(
        epochs,
        [electrode_idx],
        phoneme_pair=phoneme_pair,
        subject=subject,
        population_name=str(electrode_idx),
        global_min_sample=min_sample,
        global_max_sample=max_sample,
        stride=stride,
        window_size=window_size,
        pca_num_components=[0.1, 0.25, 0.5, 0.75, 0.9],
        target="behavior_categorical_forced",
        baseline_features=["resampled"],
        strategy="train-test",
        groupby=None,
        stratify=("resampled", "lexical_evidence"),
        return_estimators=True,
        n_jobs=n_jobs,
    )

# %% [markdown]
# ## Save

# %%
joblib.dump(
    {
        "decoding_results": A_decoding_results,
        "decoders": A_decoders,
    },
    f"{outdir}/results.joblib",
)

...
# %% [markdown]
# Ganong-effect decoding from single electrodes — causal5 HGA-only variant.
#
# Mirrors `ganong_decoding_single_electrode.py` exactly except that the decoder
...
# (`groupby=None`). Stratification on `(resampled, lexical_evidence)` balances
# acoustic steps and completions across CV folds without trial filtering.
#
# Electrode set: restricted to sites that show significant behavioral decoding in
# `behavior_decoding_single_electrode_hga_only_summarize` (full_roc_auc ≥ threshold
# at their peak window, averaged over folds). Note that this HGA-only chain
# filters by the HGA-only behavior summary — a deliberate deviation from the
# non-HGA-only ganong decoder, which reads the full-comparison summary.
#
...
#
# Load the HGA-only behavior summary. A site passes if its peak-window
# full_roc_auc (averaged over word_end groups and folds) meets the threshold
# in any phoneme pair.
...
# %% [markdown]
# ## Decode from behaviorally significant electrodes — pooled across completions (HGA-only)
#
# Key differences from `behavior_decoding_single_electrode_hga_only`:
